...
export class MailService {
  ...
  async sendOwnerInvite(
    ...
    const subject: string = `You have been invited to own ${organizationName}`;
    const html: string = this.buildInviteHtml(organizationName, inviteUrl);

    if (!this.transporter) {
      this.logger.warn(
        `SMTP is not configured — owner invite link for ${email}: ${inviteUrl}`
      );
  ...
  private buildInviteHtml(organizationName: string, inviteUrl: string): string {
    return `
      <div style="font-family: sans-serif; line-height: 1.5;">
        <h2>You have been invited to own ${organizationName}</h2>
        <p>Follow the link below to set up your password and take ownership of your organization:</p>
        <p>
          <a href="${inviteUrl}"
